...
class LSTMLyricsMelodies(nn.Module):
    """
    Bidirectional LSTM model for lyrics and melodies inputs
    Architecture: input sequence --> embedding --> concatenation layer --> Bidirectional LSTM --> dropout layer --> output
    """
    ...
    def forward(self, lyrics_input_sequence, melodies_input_sequence):
        ...
        if embedding_layer.size()[0] == 1:  # when running test phase to generate lyrics
            concatenate_features_layer = torch.cat((embedding_layer, melodies_input_sequence.unsqueeze(0).float()),
                                                   dim=2)
