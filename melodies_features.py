...
def get_melodies_features_type_T1(melodies_list, sequence_length, all_songs_lyrics_words_as_int):
    ...
    for idx, midi_file in tqdm(enumerate(melodies_list), desc="Extracting features for melodies", unit="midi file",
        ...
        num_of_words_in_curr_song = len(
            all_songs_lyrics_words_as_int[idx])  # for example idx = 67,  len([15, 1444, 8484, 0, 3, 2, ...])
        ...
        for word_idx_in_song in range(num_of_words_in_curr_song):  # iterate over every word and get the features for it
            features_for_curr_word = get_features_for_one_word(word_idx_in_song, avg_time_per_word_in_curr_song,
                                                               midi_file)
...
def get_melodies_features_type_T2(melodies_list, sequence_length, all_songs_lyrics_words_as_int):
    ...
    for midi_idx, midi_file in tqdm(enumerate(melodies_list), desc="Extracting features for melodies", unit="midi file",
        ...
        cnt_notes_per_word = int(piano_roll_matrix.shape[1] / num_of_words_in_curr_song)  # Num of piano roll columns per word
        curr_features_for_curr_song = []
        for word_i in range(num_of_words_in_curr_song):  # iterate over every word and get the features for it
            start_i = word_i * cnt_notes_per_word
            end_i = start_i + cnt_notes_per_word
            piano_roll_one = piano_roll_matrix[:, start_i:end_i]
            piano_roll_one = piano_roll_one.transpose()
            notes_features = np.sum(piano_roll_one, axis=0)  # sum columns
...
def normalize_melodies_features(features_in):
...
def create_melodies_data_sets(train_num, melodies_list, sequence_length, words_as_int_lyrics_matrix, pkl_file_path,
    ...
    if feature_extraction_type == 'T1':
        melodies_features = get_melodies_features_type_T1(melodies_list, sequence_length, words_as_int_lyrics_matrix)
    elif feature_extraction_type == 'T2':
        melodies_features = get_melodies_features_type_T2(melodies_list, sequence_length, words_as_int_lyrics_matrix)
    else:
        melodies_features = []
    ...
    melodies_train = melodies_features[:train_num]
    melodies_test = melodies_features[train_num:]
    ...
    with open(pkl_file_path, 'wb') as f:
        content = [melodies_train, melodies_test]
        pickle.dump(content, f)
        print('Dumped melodies features files')

    return melodies_train, melodies_test
